...
- ✅ **Staged climbs** (2026-06-08, CC) — `match_end_panel.gd` Ranked **Surface 2**: the LP bar fills on the settle curve (armed at the pre-match LP), the LP value pops on landing, the FINAL ORDER rows slide in staggered from the left, and a **promotion is a staged set-piece** — the bar tops out at the cap, the tier name + promotion note + LP value pop, then the bar resets and fills into the new tier (demotion mirrors it). `rail.gd` **ghost ladder**: the Current value pops when the score climbs past a rung (the lowest remaining target rises — one pop per pass, not per hit). Verified: same-tier fill by mid+settled capture (`ranked_shot`), promotion path logic-verified (ranked_lp_test green), ghost-ladder pop logic-verified (ghost_ladder_test green). Feel is a playtest item.
- ✅ **Pause / settings arrivals** (2026-06-08, CC) — `pause_menu.gd` + `settings_panel.gd` overlays now **arrive and leave** on the foundation curves: the dim fades in while the card scale-arrives; on close the card leaves + dim fades, then both hide. Promoted to a shared **`Motion.overlay_in` / `Motion.overlay_out`** (the modal-overlay grammar — one authored hand). Runs under the single-player pause via the panels' existing `PROCESS_MODE_ALWAYS`. Settings gained an explicit `_is_open` flag so it reports closed through the leave animation. Verified parse + motion_test; feel is a playtest item.

**Juice pass: all locked surfaces implemented.** Open dials remain (overshoot magnitude, hero tilt, set-piece stagger — tune in playtest) and the optional impact-event audio sting is still flagged/undecided. The Settings reduced-motion toggle still needs wiring to `Motion.reduced`.
