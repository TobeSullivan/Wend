...
- ◐ **Meta menu — home** (2026-06-08, CC) — `home_screen.gd`: the home now wears the break-the-grid attitude (Trials rides high, Ranked rides low, both tilted −2.5° off-axis) and **arrives staggered** on load: season pill + WEND title/subtitle ease in → the two heroes **drop into their off-axis offsets** (earning L) → Campaign rises → corner buttons fade in last. Each hero sits in a plain Control **slot** (the HBox manages the slot; the hero is positioned + tilted + slid freely inside, since a container would stomp its position). Verified by mid-arrival + settled captures. **Remaining meta-menu:** the **home→select screen transition** (SceneManager uses hard `change_scene_to_file`, so this needs a snapshot/overlay layer — architectural, the "connective tissue") and the **select-screen card cascade** (per-screen, contained).
