...
- **Obstacle art is board-scoped + local; footprint is seeded + shared.** The generator bakes only the blocking footprint (`map_generator` → empty `prop_id`); the prop ART is resolved at draw time by `ObstacleProps.art_for(equipped_board, footprint, cell_key)` — so a board only ever shows props that belong to it (Suburbia → its own pool; every other board → the default urban-decay pool). Pure cosmetic, never in the record. Authored `.tres` prop_id still wins for hand-placed campaign props. (2026-06-10, implemented)
- **FX is a set of HOOK points, not one overlay.** A milestone FX declares which hook(s) it uses: *body* (the projectile sprite itself — `fx_fireball`/`fx_gold_bolt`: the fire tower fires a fireball, same path/speed/size as the arrow, so the cardinal rule is about timing+footprint, not arrow-vs-fireball); *impact* (detonation/burst on hit — `fx_blue_impact`/`fx_smoke_ring`); *trail* (`fx_fire_trail`). **Impact FX are noise-gated:** kept small/short and eyeballed at scale (many towers × many shots) before commit; dial-backs = shrink, drop opacity, or detonate only on kill. SFX allowed (the arrow simply never had one). (2026-06-10) — **System + `fx_fireball` implemented** in `projectile_fx.gd` (`config_for(id)` → body/impact hooks; render-only, local board, resolved from the equipped "proj" id; crits exempt). Remaining `fx_*` are art-mapping only. **Impact is ON-KILL-ONLY** (playtest 2026-06-10: per-hit bursts stacked on a multi-tower-pounded mob and occluded it). A hit kills iff `damage >= hp` (captured pre-`take_hit`); one burst per kill, sized ~44px.
- **Color decision FINAL:** the 7 distinct owned tower sprites force it — growth/investment signal lives in a **tier aura**, the tower body is a **pure skin slot** (multishot + fire-rate + info box already carry legibility). **Aura form locked 2026-06-23: a ground GLOW (not a ring), keyed to merge tier — size + opacity + pulse intensify T2→T10; color walks a per-board ramp (warm on Suburbia, cool on the greens) so the glow reads on every board. T1 = no aura. Full spec `design/TOWER_AURA.md`. Supersedes the merge-reference "body color walks a 10-stop ramp" tier tell — body returns to a pure skin slot; barrels + tier badge + T10 gold ring stay. Render-only, tier-driven, never in the record; pulse off the sim tick.**
