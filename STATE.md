...
## Next step
- **CC:** Hetzner deploy is DONE (server live, see Current focus). Next: validate a real 2-client cross-network match, then bake the VPS IP into `lobby.gd`. After that — rebuild in-match UI from `notes/mockups/`, re-rescale board + campaign `.tres` to the confirmed ~23×14 (**not** 20×11), pending the user's tile-feel check.
