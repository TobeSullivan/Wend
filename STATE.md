...
3. **Matchmaking** (bigger; `matchmaking_orchestration.md`) — ticket + escalation → forming lobby (fill X/8, unanimous-or-auto-at-8, abstain=no, no timeout) → instant-join → hand clients the **Godot match server** address + `match_id`. Authority stays in Godot; Nakama only matchmakes + points. **Decomposed into 3a–3d:**
   ...
   - **3b. Nakama matchmaker** — client ticket → escalation (widen band, lower count floor) → matched group. New code against the live box.
   - **3c. Forming lobby + vote** — a Nakama JS match handler as the filling lobby (X/8, unanimous-or-auto-at-8, abstain=no) → emits `match_id` + server addr. New `index.js` handler + client UI.
   - **3d. Handoff + E2E** — Nakama hands (addr + match_id) → client JOIN_ROOMs the Godot server → full match → results post. Rewires `lobby.gd` onto the rooms model.
