...
const PROPS := {
	...
	# Tall stacked-stone pillar (art ~3.8 cells tall). Blocks 1x2 at the base; the cells its
	# art overhangs upward are made non-buildable LOCALLY (obstacle.gd overhang_cells +
	# build_controller.no_build) so towers don't sit on it — without baking a taller footprint
	# into the shared seed (that would force a generation change; see board_obstacle_model.md).
