...
func _build_ui() -> void:
	...
	for stat in STATS:
		...
		name_label.custom_minimum_size = Vector2(96, 36)
		name_label.vertical_alignment = VERTICAL_ALIGNMENT_CENTER
		row.add_child(name_label)

		var tier_label := _label("T0", 14, Color.WHITE)
		tier_label.custom_minimum_size = Vector2(28, 36)
...
func _build_objectives() -> void:
	if not _has_objectives():
		_objectives.add_child(_label("[B] Build  ·  click a tower to upgrade  ·  right-click to sell", 13, Color(0.7, 0.75, 0.85)))
		return
