...
signal phase_changed(phase, info)
signal lobby_update(info)
...
var _my_mmr := RankedLadder.SEED_MMR
...
func _on_go(info) -> void:
	...
	var err := SceneManager.net_join(String(info.get("host", "")))
	if err != OK:
...
func _on_server_msg(_from: int, msg: Dictionary) -> void:
	...
	if t.received.is_connected(_on_server_msg):
		t.received.disconnect(_on_server_msg)
